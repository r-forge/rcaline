...
\Rpkg{Rcaline} provides an interface to the \textsc{CALINE} family of line-source atmospheric dispersion models \cite{benson1979caline3,benson1992review}. These steady-state, Gaussian dispersion models are used to predict aerosol concentrations downwind from mobile emission source(s) such as highway traffic.

\subsection{Features}

At the heart of \Rpkg{Rcaline} is a Fortran library, \Rpkg{libcaline}, that wraps original code from the \textsc{CALINE3} implementation created by CALTRANS.\footnote{Support is planned for \textsc{CALINE4} in a future release.} Given the same inputs, \Rpkg{libcaline} has been tested to produce identical outputs. However, \Rpkg{libcaline} removes significant limitations found in previous implementations of \textsc{CALINE}. For example, \Rpkg{libcaline} can be  be used to model an unlimited number of roadway links and an unlimited number of receptors, bound only by available memory and CPU resources.

By providing access to \Rpkg{libcaline} within the R environment, \Rpkg{Rcaline} also makes it much easier to run \textsc{CALINE} using contemporary data sources, such as ESRI shapefiles, as input. \Rpkg{Rcaline} also provides full machine-precision access to \textsc{CALINE} model results in a convenient format. Thus, it is easy to use basic R commands---or third-party R packages---to visualize, compare, and export results. 
...
The \textsc{CALINE3} model is most appropriately used for modeling dispersion of carbon monoxide (CO) attributable to free-flow traffic with wind speeds greater than 1.0 m/s. As with any model, care should be exercised to ensure that the practical application is theoretically well founded. For more on the theoretical scope and limitations of the \textsc{CALINE} model family, including terrain and other considerations, see \cite{benson1992review}. 

\section{A Brief Example}

In this section, we illustrate the use of \Rpkg{Rcaline} by applying it to highway data sourced from the OpenStreetMaps project \cite{haklay2008openstreetmap} (Figure~\ref{fig:highways}). There are three distinct steps: (1) preparing the input; (2) running the model; and (3) visualizing and/or exporting the output. Most of the work consists in preparing the input, and the reader is well advised to consult the \textsc{CALINE} User's Guide \cite{benson1979caline3} for additional detail.

\subsection{Preparing the input}

\subsubsection{Reading from an ESRI shapefile}

Contemporary GIS data is often stored in the ESRI shapefile format. The \Rpkg{Rcaline} package includes an example of such a shapefile. 
...
receptor.grid <- as.data.frame(locations)
names(receptor.grid) <- c('x', 'y')
@
...
In preparation for a \textsc{CALINE} model run, we need to break the roadway geometry up into individual segments. This only needs to be done once; you can save the result for future use. 
...
summary(link.data)
save(link.data, file='highways.RData')
@
...
In this example, we're provided with an estimate of traffic volume: AADT (Annual Average Daily Traffic). However, we'll need to impute or assume default values for a few other variables. For example, the \code{LANES} variable, which supplies the number of lanes, has some NA (missing) values. This is not uncommon. We can replace those with the median of the remaining values, as follows:
...
Because we are interested in hourly estimates, we divide the average daily traffic by 24. We don't have an emission factor available, so we assume uniform emissions of 10 g /veh-mi. Precise estimates for emission factors can be obtained with the use of a sophisticated model, such as EMFAC \cite{emfac2007}, that takes into account prevailing weather conditions (temperature and humidity) as well as the composition of the local vehicle fleet. 
...
\textsc{CALINE3} requires four variables corresponding to the prevailing meteorology:
\begin{itemize}
\item wind bearing;
\item wind speed;
...
\textsc{CALINE3} also requires several ``job parameters'', including:
\begin{itemize}
\item averaging time;
...
As a quick check on the model output, it can be good to create a visualization. This most easily takes the form of a map.
